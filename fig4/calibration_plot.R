...
# Read saved results of the calibration analysis with different parameters
# and build a tibble for plotting
for (param in parameters) {
  square_size <- param$square_size
  point_nums <- param$point_nums

  for (i in seq_along(point_nums)) {
    point_num <- point_nums[i]
    file_path <- sprintf(file_path_template, square_size, point_num)

    calibration_df <- read_csv(file_path)
...
calibration_p <- ggplot(calibration_df_glob) +
  ...
  geom_text(aes(x=x, y=y, label = label, hjust = 0), data = labels_df, parse = T,
