...
output_folder <- "Power matern effect"
...
# Derived params
interaction_dist_h <- 2 * interaction_dist
interaction_dist_pcf <- interaction_dist
interaction_dist_lcf <- 2 * interaction_dist
...
output_folder <- file.path(output_folder, paste0("H1_", num_h1, "_H0_", num_h0, "_B", num_boot, "_S", square_side, "_CR", interaction_dist, "_", h1_pprocess))
dir.create(output_folder, showWarnings = FALSE)
...
for (num_points in nums_points) {
  ...
  num_points_folder <- file.path(output_folder, paste0("PN", num_points))
  dir.create(num_points_folder, showWarnings = FALSE)
  ...
  power_pcf <- NULL
  power_lcf <- NULL
  for (cluster_rad in cluster_rads) {
    tic()
    print(sprintf("Cluster radius %s", cluster_rad))

    experiment_name <- paste0("CR", cluster_rad)
    experiment_folder <- file.path(num_points_folder, experiment_name)
  ...
  power_h$stat <- "h"
  power_h$cluster_rad <- cluster_rads

  power_pcf$stat <- "pcf"
  power_pcf$cluster_rad <- cluster_rads

  power_lcf$stat <- "lcf"
  power_lcf$cluster_rad <- cluster_rads

  power_df <- rbind(power_h, power_pcf, power_lcf) %>%
  mutate(stat=factor(stat, levels=c("h", "pcf", "lcf")))

  write.csv(power_df, file.path(num_points_folder, "power.csv"))
}
